...
const Notifications = () => {
  ...
  useEffect(() => {
    const fetchNotifications = async () => {
      try {
        const response = await axios.get('http://localhost:8080/api/admin/notifications'); // Adjust the endpoint as needed
  ...
  const deleteNotification = async (id) => {
    if (window.confirm(`Are you sure you want to delete this notification?`)) {
      try {
        await axios.delete(`http://localhost:8080/api/admin/notifications/${id}`); // Call the delete endpoint
